    ...
    where TFixture : MonsterFixupTestBase<TFixture>.MonsterFixupFixtureBase, new()
{
    ...
    public async Task Composite_fixup_happens_when_FKs_change_test()
    {
        ...
        using (var context = CreateContext())
        {
            ...
            // See issue#16428 — VistaDB also returns U+FFFD for varbinary bytes > 127 via SUBSTRING.
            var clientEval = context.Database.ProviderName is "Microsoft.EntityFrameworkCore.Sqlite" or "Microsoft.EntityFrameworkCore.VistaDB";
            var productPhoto1 = clientEval
                ? context.ProductPhotos.ToList().Single(e => e.Photo[0] == 101)
                : context.ProductPhotos.Single(e => e.Photo[0] == 101);
            var productPhoto2 = clientEval
                ? context.ProductPhotos.ToList().Single(e => e.Photo[0] == 103)
                : context.ProductPhotos.Single(e => e.Photo[0] == 103);
            var productPhoto3 = clientEval
    ...
    protected void SimpleVerification()
    {
        ...
        // See issue#16428 — VistaDB also returns U+FFFD for varbinary bytes > 127 via SUBSTRING.
        if (context.Database.ProviderName is "Microsoft.EntityFrameworkCore.Sqlite" or "Microsoft.EntityFrameworkCore.VistaDB")
        {
            Assert.Equal(
                ["101", "103", "105"],
